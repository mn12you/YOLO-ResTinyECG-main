...
def handel_preds(preds, cfg, device):
    ...
    anchors = torch.from_numpy(anchors.reshape(2, cfg["anchor_num"], 2)).to(device)
...
#模型评估
def evaluation(val_dataloader, cfg, model, device, conf_thres = 0.1, nms_thresh = 0.3, iou_thres = 0.5):
